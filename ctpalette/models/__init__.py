...
def create_model(**cfg_model):
    ...
    model = init_obj(model_opt, logger, default_file_name='ctpalette.models.model', init_type='Model')

    return model

def define_network(logger, opt, network_opt):
    """ define network with weights initialization """
    net = init_obj(network_opt, logger, default_file_name='ctpalette.models.network', init_type='Network')
...
def define_loss(logger, loss_opt):
    return init_obj(loss_opt, logger, default_file_name='ctpalette.models.loss', init_type='Loss')

def define_metric(logger, metric_opt):
    return init_obj(metric_opt, logger, default_file_name='ctpalette.models.metric', init_type='Metric')
