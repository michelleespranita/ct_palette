...
import tqdm
import random
from ctpalette.core.base_model import BaseModel
from ctpalette.core.logger import LogTracker
import copy
import torch.nn as nn
from torchvision import models

class BodyBBModel(nn.Module):
    def __init__(self):
        super(BodyBBModel, self).__init__()
        self.model = models.resnet18(weights=models.ResNet18_Weights.DEFAULT)
        last_in_features = self.model.fc.in_features
        output_features = 4
        self.model.fc = nn.Linear(last_in_features, output_features)
...
class Palette(BaseModel):
    ...
    def test(self):
        ...
        with torch.no_grad():
            random_batch_idx = random.sample(list(range(0, len(self.phase_loader))), k=1)[0]
            batch_idx = 0
